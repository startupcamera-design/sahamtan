...
export const CombinedAnalysisModal: React.FC<CombinedAnalysisModalProps> = ({ stock, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchAndRenderChart = async () => {
      ...
      try {
        ...
        // -----------------------------------------------------------
        // PERBAIKAN PENTING: Pengecekan Aman (Safe Call) setMarkers
        // -----------------------------------------------------------
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4">
      <div className="bg-slate-800 border border-slate-700 w-full max-w-7xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[96vh]">
        
        {/* Header Modal */}
        <div className="px-6 py-3.5 border-b border-slate-700 flex items-center justify-between bg-slate-900/80 shrink-0">
          <div className="flex items-center gap-3">
            <BarChart2 className="w-5 h-5 text-emerald-400" />
            <h3 className="text-base sm:text-lg font-bold text-slate-100 flex items-center gap-2">
              Grafik & Analisis Saham: <span className="text-emerald-400">{stock.ticker}</span>
            </h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 flex-1 overflow-hidden">
          ...
          {/* KOLOM KIRI: GRAFIK */}
          <div className="lg:col-span-7 p-4 relative bg-slate-900/60 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-slate-700">
            
            {/* Overlay Informasi Legend AI */}
            {(stock.aiScore || stock.aiAction || stock.tradingPlan) && (
              <div className="absolute top-6 left-6 z-10 bg-slate-900/90 border border-slate-700/80 rounded-xl p-3 shadow-lg backdrop-blur-md max-w-xs space-y-2 text-xs">
          ...
          <div className="lg:col-span-5 p-5 overflow-y-auto bg-slate-900/40 max-h-[500px] lg:max-h-[580px]">
            <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-700/80">
              <BookOpen className="w-4 h-4 text-emerald-400" />
              <h4 className="text-sm font-bold text-slate-200">Narasi Laporan AI</h4>
            </div>
            ...
            {stock.aiMarkdown ? (
              ...
              <div className="text-slate-400 text-xs italic py-8 text-center">
                Laporan narasi AI tidak ditemukan.
              </div>
            )}
          </div>
        ...
        {/* Footer Modal */}
        <div className="px-6 py-3 border-t border-slate-700 bg-slate-900/80 flex justify-between items-center text-xs text-slate-400 shrink-0">
          <span>💡 Gunakan scroll wheel di atas grafik untuk Zoom dan klik-hold-drag untuk menggeser.</span>
          <button
            onClick={onClose}
